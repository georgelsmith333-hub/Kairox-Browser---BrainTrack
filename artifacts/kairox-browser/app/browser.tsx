...
export default function BrowserScreen() {
  ...
  const previewShell = (
    <ScrollView contentContainerStyle={styles.page} showsVerticalScrollIndicator={false}>
      <View style={styles.identityRow}><StatusPill label="Profile isolated" tone="good" /><StatusPill label="HTTPS first" /><StatusPill label="Preview handoff" tone="warn" /></View>
      <View style={styles.hero}>
        <Text style={[styles.heroEyebrow, { color: colors.primary }]}>KAIROX BROWSER</Text>
        <Text style={[styles.heroTitle, { color: colors.foreground }]}>A live browser surface.{'\n'}With a clear engine boundary.</Text>
        <Text style={[styles.heroText, { color: colors.mutedForeground }]}>This preview renders the requested page in a live web frame. Android uses the native Kairox WebView for deeper controls; use the external fallback whenever a site refuses to be framed.</Text>
        <View style={styles.buttonRow}>
          <Pressable onPress={openExternal} style={({ pressed }) => [styles.openButton, { backgroundColor: colors.primary, opacity: pressed ? 0.7 : 1 }]}><Ionicons name="open-outline" size={18} color={colors.primaryForeground} /><Text style={[styles.openButtonText, { color: colors.primaryForeground }]}>Open page securely</Text></Pressable>
  ...
  const nativeBrowser = (
    <View style={styles.webSurface}>
      ...
      {loadError ? <View style={styles.errorOverlay}><Ionicons name="warning-outline" size={22} color={colors.primary} /><Text style={[styles.errorTitle, { color: colors.foreground }]}>Page unavailable</Text><Text style={[styles.errorText, { color: colors.mutedForeground }]}>Use the external browser fallback or try again.</Text><Pressable onPress={() => { setLoadError(false); webViewRef.current?.reload(); }} style={[styles.retryButton, { backgroundColor: colors.primary }]}><Text style={[styles.retryText, { color: colors.primaryForeground }]}>Retry</Text></Pressable></View> : null}
